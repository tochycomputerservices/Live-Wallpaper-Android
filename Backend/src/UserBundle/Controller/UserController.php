...
class UserController extends Controller
{
   ...
   public function api_getAction(Request $request,$user,$me,$token){
            ...
            if (sha1($follower->getPassword()) == $key_) {
                if ($exists) {
                   ...
                   $message="You Unfollowing ".$user->getName(); 
                    ...
                    $message_notif = $this->send_notificationToken($tokens,$messageNotif,$key); 
                }
            }else{
                $code=500;  
    ...
    public function wallpapersAction(Request $request,$id)
    {
        $em= $this->getDoctrine()->getManager();
        $user = $em->getRepository("UserBundle:User")->find($id);
        if ($user==null) {
        ...
        $dql        = "SELECT w FROM AppBundle:Wallpaper w  WHERE w.user = ".$user->getId() ." ORDER BY w.created desc";
        $query      = $em->createQuery($dql);
        $paginator  = $this->get('knp_paginator');
        $pagination = $paginator->paginate(
            $query,
        ...
        return $this->render(
                'UserBundle:User:wallpapers.html.twig',array(
                    "pagination"=>$pagination,
                    "user"=>$user
                )
            );
    ...
    public function  api_followingstopAction(Request $request,$user,$token)
    {
        ...
        $users=array();
        foreach ($user->getUsers() as $key => $e) {
            if (sizeof($e->getWallpapers())>0) {
               $b["id"] = $e->getId();
               $b["name"] = $e->getName();
               $b["trusted"] = $e->getTrusted();
               $b["image"] = $e->getImage();
               $last_wallpaper = $em->getRepository('AppBundle:Wallpaper')->findOneBy(array("enabled"=>true,"user"=>$e),array("created"=>"desc"));
               if ($last_wallpaper!=null) {
                    $b["wallpaper"] = $last_wallpaper;
                    $users[]= $b;
               }
    ...
    public function api_searchAction(Request $request,$query,$token)
    {
        if ($token!=$this->container->getParameter('token_app')) {
            throw new NotFoundHttpException("Page not found");  
        }
        ...
        $qb = $repo->createQueryBuilder('u')->where("u.name like '%".$query."%'");
